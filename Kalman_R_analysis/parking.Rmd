...
task_df <- read_csv("parking_all_errors.csv")
...
task_df$KF <- factor(task_df$KF, levels = kf_order)
...
one.way <- task_df %>%
  ...
  anova_test(dv = trans_err, wid = sample_id, within = KF) %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
one.way
...
fit <- lm(trans_err ~ distance * KF, data = task_df)
summary(fit)

uchicago_palette <- c("#800000", "#767676", "#FFA319")

plt <- interact_plot(fit, pred = distance, modx = KF,
